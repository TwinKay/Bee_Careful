...
class ObjectDetectionUtils:
    ...
    def preprocess(self, image: np.ndarray, model_w: int, model_h: int):
        img_h, img_w, _ = image.shape
        scale = min(model_w / img_w, model_h / img_h)
        new_w, new_h = int(img_w * scale), int(img_h * scale)
        image_resized = cv2.resize(image, (new_w, new_h), interpolation=cv2.INTER_CUBIC)

        padded = np.full((model_h, model_w, 3), self.padding_color, dtype=np.uint8)
    ...
    def extract_detections(self, input_data, threshold: float = 0.5) -> dict:
        boxes, scores, classes = [], [], []
        num_detections = 0

        if input_data is None or isinstance(input_data, (float, np.floating, int)) or not hasattr(input_data, '__iter__'):
            print("[WARNING] Invalid input_data for extract_detections():", type(input_data))
            return {
                'detection_boxes': [],
                'detection_classes': [],
                'detection_scores': [],
        ...
        for i, detection in enumerate(input_data):
            if detection is None or not hasattr(detection, '__iter__'):
                continue
            ...
            for det in detection:
                ...
                try:
                    ...
                    print("[ERROR] Failed to parse det:", det, "Error:", e)
                    continue
