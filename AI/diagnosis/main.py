...
async def diagnose_beehive(body: DiagnosisRequest = Body(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Original image not found in S3 with key: {original_s3_key}",
        )
    except PermissionError:
        print(f"PermissionError for key '{original_s3_key}'.")
        raise HTTPException(
            status_code=403,
            detail=f"Access denied for original S3 object with key: {original_s3_key}",
        )
        ...
        print(
            f"An unexpected error occurred during S3 download for key '{original_s3_key}': {e}"
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to download original image from S3: An unexpected error occurred.",
        )
    ...
    try:
        ...
        if img_np is None:
            print(
                f"Failed to decode image from S3 (key: {original_s3_key}). File might be corrupted or not a supported image format."
            )
            raise HTTPException(
                status_code=415,
                detail="Unsupported image format or corrupt image downloaded from S3.",
            )
    except Exception as e:
        print(f"Error decoding image (key: {original_s3_key}): {e}")
        raise HTTPException(
            status_code=415, detail=f"Could not decode image from S3: {e}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="An internal error occurred during image analysis."
        )
    ...
    if not ok:
        print(
            f"Failed to encode annotated image to JPEG (original key: {original_s3_key})."
        )
        raise HTTPException(
            status_code=500, detail="Failed to encode annotated image to JPEG."
        )
    ...
    ):  # Fallback if original key was strange (e.g. just "folder/")
    ...
    try:
        ...
        print(
            f"PermissionError uploading annotated image to S3 key '{annotated_s3_key}'."
        )
        raise HTTPException(
            status_code=403,
            detail=f"Access denied for uploading annotated image to S3.",
        )
    except RuntimeError as e:
        print(
            f"RuntimeError from s3_handler during upload (key: {annotated_s3_key}): {e}"
        )
        raise HTTPException(
            status_code=503, detail=f"S3 service error during upload: {e}"
        )
    except Exception as e:
        print(
            f"Unexpected error during S3 upload for annotated image (key: {annotated_s3_key}): {e}"
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to upload annotated image to S3: An unexpected error occurred.",
        )
...
if __name__ == "__main__":
    ...
    if MODEL is None:
        print(
            "CRITICAL: Application cannot start because the YOLO model failed to load. Please check logs."
        )
    else:
