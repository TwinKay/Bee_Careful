...
try:
    ...
    if S3_ACCESS_KEY_ID and S3_SECRET_ACCESS_KEY:
        ...
        print(
            "Attempting to initialize S3 client using S3_ACCESS_KEY and S3_SECRET_KEY environment variables."
        )
    else:
        print(
            "S3_ACCESS_KEY and/or S3_SECRET_KEY not set. S3 client will use default credential chain (e.g., IAM role, ~/.aws/credentials)."
        )
    ...
    if s3_client:
        region_message = (
            f" (Region: {AWS_REGION})"
            if AWS_REGION
            else " (Region: default from AWS config/env or client args)"
        )
        if not S3_BUCKET_NAME:
            print(
                f"S3 client initialized{region_message}. CRITICAL WARNING: S3_BUCKET_NAME environment variable is not set or is empty. S3 operations will fail."
            )
        else:
            print(
                f"S3 client initialized. Configured to use bucket: '{S3_BUCKET_NAME}'{region_message}."
            )
    ...
    print(
        "ERROR: AWS credentials not found or invalid. Please configure your AWS credentials. s3_client remains None."
    )
except PartialCredentialsError:
    print(
        "ERROR: Incomplete AWS credentials found. Please check your AWS configuration. s3_client remains None."
    )
except (
    Exception
) as e:  # Catch other boto3 client initialization errors (e.g., invalid region, invalid keys if provided)
...
if not S3_BUCKET_NAME:
    ...
    raise Exception(
        "CRITICAL ERROR: S3_BUCKET_NAME environment variable is not set. This is required for the application."
    )
...
def get_s3_object_bytes(s3_key: str) -> bytes:
    ...
    if s3_client is None:
        raise RuntimeError(
            "S3 client is not initialized. Check AWS credentials and S3 client initialization logs."
        )
    ...
    try:
        ...
        print(
            f"Successfully downloaded {len(object_content)} bytes from s3://{S3_BUCKET_NAME}/{s3_key}"
        )
        ...
        if error_code == "NoSuchKey":
            ...
            raise FileNotFoundError(
                f"Object not found in S3: s3://{S3_BUCKET_NAME}/{s3_key}"
            )
        elif error_code == "AccessDenied":
            print(
                f"Error: Access denied for S3 object - s3://{S3_BUCKET_NAME}/{s3_key}"
            )
            raise PermissionError(
                f"Access denied for S3 object: s3://{S3_BUCKET_NAME}/{s3_key}"
            )
        else:
            print(
                f"S3 ClientError when trying to get object {s3_key} from bucket {S3_BUCKET_NAME}: {e}"
            )
            raise Exception(f"S3 error getting object {s3_key}: {e}")
    except Exception as e:
        print(
            f"An unexpected error occurred while downloading from S3 (key: {s3_key}): {e}"
        )
        raise


def put_s3_object_bytes(
    s3_key: str, object_bytes: bytes, content_type: str = "application/octet-stream"
) -> str:
    """
    ...
    print(
        f"Attempting to upload to {full_s3_path} ({len(object_bytes)} bytes, Content-Type: {content_type})"
    )
    try:
        ...
        if error_code == "AccessDenied":
            ...
            print(
                f"S3 ClientError when trying to put object {s3_key} to bucket {S3_BUCKET_NAME}: {e}"
            )
            raise Exception(f"S3 error putting object {s3_key}: {e}")
    except Exception as e:
        print(
            f"An unexpected error occurred while uploading to S3 (key: {s3_key}): {e}"
        )
        raise
...
if __name__ == "__main__":
    ...
    if s3_client is None:
        print(
            "CRITICAL: S3 client is not initialized (check AWS credentials/config). Most tests will be skipped or will fail."
        )

    if (
        S3_BUCKET_NAME and s3_client
    ):  # Proceed with tests only if basic config seems okay
        ...
        test_upload_data = f"This is a test file uploaded by s3_handler.py self-test at {__import__('datetime').datetime.now()}.".encode(
            "utf-8"
        )
        uploaded_s3_path = ""
        try:
            uploaded_s3_path = put_s3_object_bytes(
                test_upload_key, test_upload_data, content_type="text/plain"
            )
            ...
            try:
                ...
                if downloaded_bytes == test_upload_data:
                    print(
                        f"Download test successful for key: {test_upload_key}. Content matches."
                    )
                else:
                    print(
                        f"Download test for key: {test_upload_key}. Content MISMATCH!"
                    )
            except Exception as e:
                print(f"Error during download test for '{test_upload_key}': {e}")
        except Exception as e:
            print(
                f"An error occurred during the S3 upload/download test for key '{test_upload_key}': {e}"
            )
    else:
        print(
            "Basic S3 configuration (S3_BUCKET_NAME or s3_client) is missing or failed. Skipping detailed S3 operation tests."
        )
