...
type UseBeehivePositionPropsType = {
  containerRef: React.RefObject<HTMLDivElement | null>;
  scale: number;
};

export const useBeehivePosition = ({ containerRef, scale }: UseBeehivePositionPropsType) => {
  /**
   * 맵 중앙에서 빈 위치 찾기
   */
  const findCenterPosition = useCallback(
    (hives: BeehiveType[]): { xDirection: number; yDirection: number } => {
      ...
      const { x, y } = findEmptyPosition(hives);
  ...
  const findOptimalPosition = useCallback(
    (hives: BeehiveType[]): { xDirection: number; yDirection: number } => {
      ...
      // 화면 영역 내에서 빈 위치 찾기
      const { x, y } = findVisibleEmptyPosition(hives, visibleArea);

      // 충돌 감지를 한 번 더 확인 (안전 장치)
      if (isPositionOccupied(x, y, hives)) {
        // 충돌이 감지되면 맵 전체에서 빈 위치 찾기
        console.log('충돌 감지됨, 맵 전체에서 빈 위치 찾기');
