  ...
  const parsedData = useMemo(() => {
    const result = {
      ...
      imagoDisease: [
        {
          name: '진드기(응애)',
          value: data.diagnosis.imago?.varroaCount,
          ratio: data.diagnosis.imago?.varroaRatio,
          color: '#E57373',
        },
        {
          name: '날개바이러스',
          value: data.diagnosis.imago?.dwvCount,
          ratio: data.diagnosis.imago?.dwvRatio,
          color: '#FFB74D',
        },
        {
          name: '정상',
          value:
            data.imagoCount -
            (data.diagnosis.imago?.varroaCount || 0) -
            (data.diagnosis.imago?.dwvCount || 0),
          ratio: 100 - (data.diagnosis.imago?.varroaRatio || 0),
          color: '#E6E6E6',
        },
      ],
      larvaDisease: [
        {
          name: '진드기(응애)',
          value: data.diagnosis.larva?.varroaCount,
          ratio: data.diagnosis.larva?.varroaRatio,
          color: '#E57373',
        },
        {
          name: '부저병',
          value: data.diagnosis.larva?.foulBroodCount,
          ratio: data.diagnosis.larva?.foulBroodRatio,
          color: '#64B5F6',
        },
        {
          name: '석고병',
          value: data.diagnosis.larva?.chalkBroodCount,
          ratio: data.diagnosis.larva?.chalkBroodRatio,
          color: '#81C784',
        },
        {
          name: '정상',
          value:
            data.larvaCount -
            (data.diagnosis.larva?.varroaCount || 0) -
            (data.diagnosis.larva?.foulBroodCount || 0) -
            (data.diagnosis.larva?.chalkBroodCount || 0),
          ratio: 100 - (data.diagnosis.larva?.varroaRatio || 0),
          color: '#E6E6E6',
        },
      ],
    };
